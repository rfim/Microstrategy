//==============================================================================================
//	FILENAME	:	ChartCanvas.h
//	AUTHOR		:	Xinpan Xiao
//	CREATION	:	2010-11-12
//	Copyright (C) MicroStrategy Incorporated 2010
//	All Rights Reserved
//==============================================================================================
#ifndef MsiChart_ChartCanvas_h
#define MsiChart_ChartCanvas_h 
...
namespace MsiChart
{
	...
	/**
	 This is an abstract class that provides a drawing interface with a set drawing instructions. 
	 The ChartContext contains a pointer to this class, and all actual drawing instructions are eventually paased to this class.
	 By inheriting and implementing this class, one can accommodate with different APIs.
	 For example, the class JSONCanvas inherits from this class, and it stores all drawing instructions into a JSONString.
	 */
	...
	public:
		ChartCanvas();
		ChartCanvas(ChartContext *ipContext);
		virtual ~ChartCanvas();

		/**
		 An interface that converts the current canvas into a string and stores into orGraphDesc.
		 @param orGraphDesc stores the output string. Actual contents and formats of this string depends on the actual implementation class that inherits from ChartCanvas.
		 */
		virtual bool SaveToString(std::string& orGraphDesc) = 0;
#ifdef _VE_CE
		/**
		 An interface that converts the current canvas into vector of VE objects.
		 */
		virtual bool SaveToVEObjects(std::vector<MVE::VEFormatList>& ioFormatList, std::vector<MVE::VEUnitList>& ioUnitList) = 0;
#endif
		/**
		...
		**/
		...
		virtual void ApplyBeveledFrameToRectangle(const Rect2D& irRect, double iRotationAngle, short iBevelDepth, 
												  ...
												  const RGBColor& irRBColor, short iRBAlpha) = 0;
		...
		/**
		 Specifies the ChartContext to which this ChartCanvas is attached.
		 @param ipContext points to the ChartContext in which this ChartCanvas is used.
		 */
		virtual void SetContext(ChartContext *ipContext);
		
		/**
		 Returns the ChartContext to which this ChartCanvas is attached.
		 @return a pointer to the ChartContext that this ChartCanvas works with.
		 */
